...
const createMdLink = (text, link, ogp = false) =>
	`[${text}](${ogp ? link : link.replace(/^https?:/, "")})`;
// PR も format が同じなので PR のリンクも作れる
const createIssueLink = (issue, ogp = false) =>
	createMdLink(issue.title, issue.html_url || issue.url, ogp);
const createRepoLink = (repo, ogp = false) =>
	createMdLink(repo.name, repo.html_url, ogp);
const createUserLink = (user, ogp = false) =>
	createMdLink(user.login, user.html_url, ogp);
		...
		if (payload.action === "submitted") {
			if (payload.review.state === "approved") {
				...
				message = [
					...
					...(payload.review.body === null || payload.review.body.length === 0
						? []
						: ["", "---", payload.review.body]),
				].join("\n");
			}
